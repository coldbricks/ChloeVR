...
| **R4** (dancer self-shadow, depth-texture hw-PCF, auto-fit frustum) | **IMPLEMENTED 2026-06-10 (night) — items 1-4, build-verified both flavors, installed on Galaxy XR. On-head verification + Quest 3 pass pending; capsule-AO bonus (item 5) deferred. 4 review majors fixed (stale-pose shadow map, world-scale slope bias, world-invariant PCSS kernels, texel snap) — see R4 STATUS + NOTES.md.** |
...
**STATUS: IMPLEMENTED 2026-06-10 (night) — items 1-4 landed, build-verified both flavors, installed on Galaxy XR; ON-HEAD VERIFICATION PENDING (acceptance test per the aesthetic-bar note in NOTES.md: re-run the Chelsea no-PBR vs full-PBR A/B). Item 5 (per-bone capsule AO) deliberately deferred as a follow-on. Single-file change (GlesModelRenderer.kt only — the fit lives in the renderer, so FilamentModelActivity needed nothing).** Implementation decisions: (a) auto-fit treats shadowSpread as a CAP (`min(fit, spread)`) — slider meaning and scene persistence untouched; (b) compare state lives on the TEXTURE; raw PCSS blocker reads use ONE compare-off NEAREST sampler object on unit 1, unbound after every grid/SP pass (a stale sampler object would poison the next pass's normal-map unit); (c) lookAt target moved from y=0 to the AABB center — a tight frustum centered at the floor would clip a standing dancer's head; (d) the model program's shadow-sampler unit is assigned UNCONDITIONALLY (a sampler2DShadow left on default unit 0 type-clashes with uBaseColor → INVALID_OPERATION → black models whenever shadows are off/fallen back). Adversarial review (4 lenses × 3 refuters/finding, 46 agents) confirmed 4 distinct majors beyond the audit spec, all fixed + re-build-verified: (1) the shadow pass rendered LAST frame's pose — the joint-palette UBO upload now happens in the shadow pass (also killed the per-eye `allocateDirect`, a pre-existing Invariant-6 violation); (2) the spec's slope-bias NDC constants were 15-60mm of WORLD bias against the fixed 0.1..30 ortho window — re-expressed as 1.5mm·tanθ capped 6mm; (3) PCSS kernels are texel-relative, so auto-fit silently hardened floor softness ~4.6× and popped at quantization steps — new uShadowKernel uniform keeps them world-invariant, preserving persisted scenes' look; (4) the fitted center follows breathing joints (D3) — the light matrix is now texel-snapped or every shadow edge shimmers.

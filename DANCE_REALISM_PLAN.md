...
**PHASE A STATUS: ✅ VERIFIED ON GALAXY XR 2026-06-10 (same night) — user verdict "soooo amazing / impressed af"; no spinning/gull-wing/fold-throughs observed; presets finally read as distinct dancers. Bonus verdict: the Tier-3 shader glute push reads as a balloon next to mocap → now defaults OFF on every preset (D10 spring bones promoted as the flesh path); arch cleared, keeps authored values. Still open: scene round-trip with a layer preset, 5-min drift soak, multi-dancer perf, the whole Quest 3 pass.** Implementation record: extractor axis-correction baked (tools/mixamo_extract/bind_pose.py), all 31 presets regenerated (623 corrected JointDrive rows), Tier-4 arm gate written (real, was comment-only), layer.evaluate() ENABLED. Both bind inputs ground-truth-validated offline (GLB chains vs inverseBindMatrices 0.000°; FBX PreRotation chains vs Deformer TransformLink 0.000°); the predicted forearm Z→X flex-axis migration confirmed in the regenerated data. Key deviation from the spec: the layer now OWNS its 9 joints outright (evaluate() re-bases the full owned set to bind each frame, then composes) instead of composing on top of Tier-4/groove — compose reads live localPose, so without the re-base the gated-off arm joints accumulate rotation frame-over-frame; the groove/counter-twist seasoning on owned joints is superseded by design (mocap spine motion is already hips-relative — the hand-tuned counters would double-count). Review (spend-limit-truncated, 4 confirmed) fixes landed: scene-load now rehydrates jointLayer by preset name (was silently restoring Tier-4 arms); the arm gate requires layer.ownsArms(skel) so alias-named rigs keep Tier-4 (else groove composes spin the arms unboundedly); evaluate uses snap.phase1bar (not the phaseAtSnap rate-1 volatile fallback); per-model layer instances (shared drives array) end cache thrash + skeleton retention. Phase B (EDGE clips) not started.**
